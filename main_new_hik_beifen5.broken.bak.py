...
class MewtocolPLC:
    def __init__(self):
        ...
        self._buffer_size = 50 * 1024 * 1024
    ...
    def get_frame(self, timeout=1000):
        ...
        ret = self.cam.MV_CC_GetOneFrameTimeout(
            self._data_buf, self._buffer_size, st_frame_info, timeout
        )
        if ret != MV_OK:
            ...
            if ret != MV_OK:
                raise Exception(f"??????????????????: 0x{ret:X}")
        w = st_frame_info.nWidth
        h = st_frame_info.nHeight
        pt = st_frame_info.enPixelType
        if pt == PixelType_Gvsp_RGB8_Packed:
            img = np.frombuffer(self._data_buf, dtype=np.uint8, count=w * h * 3).reshape(h, w, 3)
            frame = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        elif pt == PixelType_Gvsp_BGR8_Packed:
            img = np.frombuffer(self._data_buf, dtype=np.uint8, count=w * h * 3).reshape(h, w, 3)
            frame = img
        elif pt == PixelType_Gvsp_Mono8:
            img = np.frombuffer(self._data_buf, dtype=np.uint8, count=w * h).reshape(h, w)
            frame = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        else:
            raise Exception(f"????????????????????????: 0x{pt:X}")
        return frame
...
class MainWindow(QMainWindow):
    ...
    def _run_yolo(self, yolo_model, frame: np.ndarray, **kwargs):
        ...
        kwargs.setdefault("imgsz", YOLO_IMGSZ)
            ...
            for xy in masks.xy:
                if len(xy) >= 3:
                    ...
                    if cv2.contourArea(contour) >= ai_min_area:
                        significant = True
                        break
        if significant and RENDER_DETECTION_OVERLAY:
            plot_bgr = cv2.cvtColor(results[0].plot(), cv2.COLOR_RGB2BGR)
            return plot_bgr, False
        return frame.copy(), (not significant)
            ...
            for xy in masks.xy:
                if len(xy) >= 3:
                    ...
                    if cv2.contourArea(contour) >= min_area:
                        has_mask = True
                        break
        if has_mask and RENDER_DETECTION_OVERLAY:
            plot_bgr = cv2.cvtColor(results[0].plot(), cv2.COLOR_RGB2BGR)
            return plot_bgr, False
        return frame.copy(), (not has_mask)
    ...
    def traditional_detect_with_params(self, src: np.ndarray, tgray: int, tarea: int, tgray1: int, tarea1: int):
        ...
        return src.copy(), (len(contours_final) == 0)
