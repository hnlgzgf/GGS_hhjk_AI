...
class DetectionThread(QThread):
    ...
    def run(self):
        try:
            ...
            roi = self.window.crop_front_roi(frame, self.params.get("front_roi"))
...
class SideDetectionThread(QThread):
    ...
    def run(self):
        try:
            ...
            roi = self.window.crop_side_roi(frame, self.params.get("side_roi"))
            if roi is None or roi.size == 0:
...
class ImageDetectionThread(QThread):
    ...
    def __init__(self, window: "MainWindow", file_path: str):
        super().__init__()
        self.window = window
    ...
    def run(self):
        try:
            ...
            ai_start = time.time()
            yolo_img, ai_ok = self.window.yolo_detect(frame.copy())
            ai_time = time.time() - ai_start
            if self.isInterruptionRequested():
                return
            trad_start = time.time()
            trad_img, trad_ok = self.window.traditional_detect(frame.copy())
            trad_time = time.time() - trad_start
            if self.isInterruptionRequested():
                return
            final_ok = bool(ai_ok and trad_ok)
...
side_model = YOLO(SIDE_MODEL_PATH, task='segment')

def _run_yolo(yolo_model, frame: np.ndarray, **kwargs):
    # ONNX 后端由 onnxruntime provider 决定是否走 GPU；不要传 torch device，避免 cpu-only torch 报错
    if USE_ONNX_MODELS:
        # 记录 ONNX 模型推理设备信息
        providers = ort.get_available_providers()
        device_info = f"ONNX Runtime Providers: {providers}"
        print(device_info)
        # 将设备信息添加到日志中
        # 注意：此时主窗口对象尚未创建，因此无法直接访问日志组件
        # 设备信息已通过 print 输出到控制台
        return yolo_model(frame, **kwargs)
    ...
    def get_frame(self, timeout=1000):
        ...
        if ret != MV_OK:
            if ret in (0x80000002, 0x80000003):
                ret = self.cam.MV_CC_GetOneFrameTimeout(data_buf, len(data_buf), st_frame_info, timeout)
            if ret != MV_OK:
                raise Exception(f"获取图像失败?x{ret:X}")
...
class MainWindow(QMainWindow):
    ...
    def _plc_poll(self):
        if not self.plc or not getattr(self.plc, "is_connected", False):
            return
        try:
            front_trigger_addr = self._get_plc_addr(self.plc_trigger_front_edit, "R0000")
            side_trigger_addr = self._get_plc_addr(self.plc_trigger_side_edit, "R0001")
            self.plc.read_bits(front_trigger_addr, 1)
            self.plc.read_bits(side_trigger_addr, 1)
            front_idx = self._r_index_from_addr(front_trigger_addr)
            side_idx = self._r_index_from_addr(side_trigger_addr)
            b0 = self.plc.get_r(front_idx) if front_idx is not None else False
            b1 = self.plc.get_r(side_idx) if side_idx is not None else False
            # 相机一触发 R0
    ...
    def _init_ui(self):
        ...
        edits = [self.conf_edit, self.ai_area_edit, self.tgray_edit, self.tarea_edit, self.tgray1_edit, self.tarea1_edit, self.side_conf_edit, self.side_area_edit]
        ...
        sliders = [self.conf_slider, self.ai_area_slider, self.tgray_slider, self.tarea_slider, self.tgray1_slider, self.tarea1_slider, self.side_conf_slider, self.side_area_slider]

        ranges = [(0, 100), (0, 10000), (0, 255), (0, 500), (0, 255), (0, 500), (0, 100), (0, 5000)]
        range_texts = ["0-1", "0-10000", "0-255", "0-500", "0-255", "0-500", "0-1", "0-5000"]
        factors = [100, 1, 1, 1, 1, 1, 100, 1]
        ...
        plc_layout.addRow("正面触发拍照:", self.plc_trigger_front_edit)
        plc_layout.addRow("侧面触发拍照:", self.plc_trigger_side_edit)
        plc_layout.addRow("正面结果信号:", self.plc_result_front_edit)
        plc_layout.addRow("正面完成信号:", self.plc_done_front_edit)
        plc_layout.addRow("侧面结果信号:", self.plc_result_side_edit)
        plc_layout.addRow("侧面完成信号:", self.plc_done_side_edit)
    ...
    def _crop_by_roi(self, src: np.ndarray, roi_values, rotate_code=None) -> np.ndarray:
    ...
    def rotate_and_crop(self, src: np.ndarray) -> np.ndarray:
        return self.crop_front_roi(src, self.get_front_roi_values())

    def rotate_and_crop_side(self, src: np.ndarray) -> np.ndarray:
        """侧面相机ROI裁剪(不旋转)"""
        return self.crop_side_roi(src, self.get_side_roi_values())

    def crop_front_roi(self, src: np.ndarray, roi_values=None) -> np.ndarray:
        return self._crop_by_roi(src, roi_values or self.get_front_roi_values(), rotate_code=cv2.ROTATE_180)

    def crop_side_roi(self, src: np.ndarray, roi_values=None) -> np.ndarray:
        return self._crop_by_roi(src, roi_values or self.get_side_roi_values(), rotate_code=None)

    def yolo_detect_with_params(self, frame: np.ndarray, conf: float, ai_min_area: int):
        results = _run_yolo(model, frame, conf=conf, iou=0.9, verbose=False)
    ...
    def side_detect_with_params(self, frame: np.ndarray, conf: float, min_area: int):
        ...
        results = _run_yolo(side_model, frame, conf=conf, iou=0.9, verbose=False)
        plot_rgb = results[0].plot()
        plot_bgr = cv2.cvtColor(plot_rgb, cv2.COLOR_RGB2BGR)
        has_mask = False
        masks = results[0].masks
    ...
    def traditional_detect_with_params(self, src: np.ndarray, tgray: int, tarea: int, tgray1: int, tarea1: int):
        if len(src.shape) == 3:
            ...
            gray = src.copy()
        results = _run_yolo(seg_model, src, conf=0.25, iou=0.7, verbose=False)[0]
    ...
    def _plc_report_result(self, final_ok: bool):
        """正面检测结果 R0501 完成脉冲, R0502 结果 (NG=1, OK=0) """
        if not self.plc:
            return
        try:
            result_addr = self._get_plc_addr(self.plc_result_front_edit, "R0502")
            done_addr = self._get_plc_addr(self.plc_done_front_edit, "R0501")
            self.plc.write_bit(result_addr, (not final_ok))
            self.plc.write_bit(done_addr, True)
            QTimer.singleShot(100, lambda: self.plc.write_bit(done_addr, False) if self.plc else None)
        except Exception as e:
    ...
    def _plc_report_side_result(self, side_ok: bool):
        """侧面检测结果 R0503 完成脉冲, R0504 结果 (NG=1, OK=0) """
        if not self.plc:
            return
        try:
            result_addr = self._get_plc_addr(self.plc_result_side_edit, "R0504")
            done_addr = self._get_plc_addr(self.plc_done_side_edit, "R0503")
    ...
    def on_image_side_detect(self):
        if self._plc_busy2:
            self.log_text.append("打开图片侧面检测被忽略: 侧面检测进行中")
            return
        file_path, _ = QFileDialog.getOpenFileName(
            self, "打开图片侧面检测", "", "Image Files (*.png *.jpg *.jpeg *.bmp *.tiff)"
        )
        if not file_path:
            return
        frame = cv2.imread(file_path)
        if frame is None:
            self.log_text.append("打开图片失败")
            return
        try:
            params = {
                "side_conf": float(self.side_conf_edit.text()),
        ...
        try:
            ai_start = time.time()
            side_img, side_ok = self.side_detect_with_params(
                roi.copy(),
                conf=params["side_conf"],
                min_area=params["side_min_area"],
            )
            ai_time = time.time() - ai_start
            side_img = draw_chinese_text(side_img, "AI侧面检测", (10, 10), 20, (0, 255, 0), 2)
            self.side_window.setPixmap(cv2_to_qpixmap(side_img))
            self.update_result_text(self.text_side, bool(side_ok))
            self.update_side_stats(bool(side_ok))
            self._plc_report_side_result(bool(side_ok))
            self.log_text.append(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 图片侧面检测完成 | "
                f"图片: {os.path.basename(file_path)} | 结果: {'OK' if side_ok else 'NG'} | "
                f"耗时: {ai_time:.3f}s"
            )
        finally:
            self._plc_busy2 = False
