...
class MainWindow(QMainWindow):
    ...
    def _update_status(self):
        ...
        if hasattr(self, "label_r1205_status"):
            ...
            self.label_r1205_status.setStyleSheet(
                f"color: {'#2ecc71' if self.plc_prev_trigger else '#555'}; font-size: 11px; font-weight: bold;"
            )
        if hasattr(self, "label_r1208_status"):
            ...
            self.label_r1208_status.setStyleSheet(
                f"color: {'#2ecc71' if self.plc_prev_trigger2 else '#555'}; font-size: 11px; font-weight: bold;"
            )
    ...
    def _on_detection_result(self, r: dict):
        try:
            final_ok = r["final_ok"]
            yolo_img = draw_chinese_text(r["yolo_img"], "AI检测安装到位", (10, 10), 20, (0, 255, 0), 1)
            trad_img = draw_chinese_text(r["trad_img"], "白点灰尘检测", (10, 10), 20, (0, 255, 0), 1)
            ...
            if r.get("save_error"):
                self._append_log(f"图像保存失败(front): {r['save_error']}")
            if r.get("saved_raw_path"):
                self._append_log(f"已保存原图: {r['saved_raw_path']}")
            if r.get("saved_roi_path"):
                self._append_log(f"已保存ROI: {r['saved_roi_path']}")
        finally:
            self._plc_busy = False
            self._detect_thread = None
    ...
    # [FIX-3] 返回 QWidget 而非 QHBoxLayout, 与 QFormLayout.addRow() 兼容
    ...
    def _start_side_detection(self, source: str = "MANUAL"):
        if self._plc_busy2:
            self._append_log(f"{source}触发被忽略，侧面检测进行中")
            return
        self._plc_busy2 = True
        try:
            t = self.get_side_roi_transform()
            params = {
                ...
                "side_rotate_angle": float(t["rotate_angle"]),
                "side_flip_horizontal": bool(t["flip_horizontal"]),
                "side_flip_vertical": bool(t["flip_vertical"]),
                "save_raw_image": self.save_raw_image_checkbox.isChecked(),
                "save_roi_image": self.save_roi_image_checkbox.isChecked(),
            }
        except Exception as e:
        ...
        self._append_log(f"{source} 启动侧面检测...")
        if self._side_detect_thread:
            try:
                self._side_detect_thread.result_ready.disconnect()
                self._side_detect_thread.error.disconnect()
            except Exception:
                pass
    ...
    def _on_side_detection_result(self, r: dict):
        try:
            ...
            if r.get("save_error"):
                self._append_log(f"图像保存失败(side): {r['save_error']}")
            if r.get("saved_raw_path"):
                self._append_log(f"已保存原图: {r['saved_raw_path']}")
            if r.get("saved_roi_path"):
                self._append_log(f"已保存ROI: {r['saved_roi_path']}")
    ...
    def on_image_detect(self):
        ...
        try:
            ...
            return
        self._append_log(f"图片检测 - 推理设备: {get_configured_infer_device_label()}")
        self.btn_image_detect.setEnabled(False)
        if self._image_detect_thread:
            try:
                self._image_detect_thread.result_ready.disconnect()
                self._image_detect_thread.error.disconnect()
            except Exception:
                pass
    ...
    def on_image_side_detect(self):
        if self._image_side_detect_thread and self._image_side_detect_thread.isRunning():
            self._append_log("图片侧面检测正在进行，请稍候。")
            return
        if self._plc_busy2:
            self._append_log("图片侧面检测忙碌：有侧面检测任务正在执行")
            return
        file_path, _ = QFileDialog.getOpenFileName(
        ...
        try:
            ...
            self._append_log(f"侧面检测参数错误: {e}")
            return
        self._append_log(f"图片侧面检测 - 推理设备: {get_configured_infer_device_label()}")
        self._plc_busy2 = True
        self.btn_image_side_detect.setEnabled(False)
        if self._image_side_detect_thread:
            try:
                self._image_side_detect_thread.result_ready.disconnect()
                self._image_side_detect_thread.error.disconnect()
            except Exception:
                pass
        self._image_side_detect_thread = ImageSideDetectionThread(
            self, file_path, side_conf=side_conf, side_min_area=side_min_area
        )
