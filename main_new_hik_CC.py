...
from ctypes import c_ubyte, cast, POINTER, CFUNCTYPE, c_void_p
...
class HiKCamera:
    ...
    def _on_image_callback(self, pData, pFrameInfo, pUser):
        try:
            st_frame_info = pFrameInfo.contents
            w = st_frame_info.nWidth
            h = st_frame_info.nHeight
            pt = st_frame_info.enPixelType
            datalen = st_frame_info.nFrameLen
            
            # 使用 ctypes 从指针获取数据流
            buf = (c_ubyte * datalen).from_address(cast(pData, c_void_p).value)
            raw_data = np.frombuffer(buf, dtype=np.uint8)
            
            if pt == PixelType_Gvsp_RGB8_Packed:
                img = raw_data.reshape((h, w, 3))
                self.last_frame = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            elif pt == PixelType_Gvsp_BGR8_Packed:
                self.last_frame = raw_data.reshape((h, w, 3)).copy()
            elif pt == PixelType_Gvsp_Mono8:
                img = raw_data.reshape((h, w))
                self.last_frame = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    ...
    def _open_device_with_info(self, dev, label: str = ""):
        ...
        # 注册回调函数
        # 海康 SDK 回调函数定义：pData, pFrameInfo, pUser
        CALLBACK_FUNC = CFUNCTYPE(None, POINTER(c_ubyte), POINTER(MV_FRAME_OUT_INFO_EX), c_void_p)
        self._callback_func = CALLBACK_FUNC(self._on_image_callback)
        ret = self.cam.MV_CC_RegisterImageCallBackEx(self._callback_func, None)
        if ret != MV_OK:
            raise Exception(f"注册回调失败: 0x{ret:X}")
            
        # 设置触发模式为 ON
        ret = self.cam.MV_CC_SetEnumValue("TriggerMode", MV_TRIGGER_MODE_ON)
        if ret != MV_OK:
            print("警告: 设置触发模式 ON 失败")
        
        # 设置触发源为 Software
        ret = self.cam.MV_CC_SetEnumValue("TriggerSource", MV_TRIGGER_SOURCE_SOFTWARE)
        if ret != MV_OK:
            print("警告: 设置触发源为 Software 失败")
    ...
    def get_frame(self, timeout=2000):
        ...
        if ret != MV_OK:
            raise Exception(f"执行软触发失败: 0x{ret:X}")
            
        # 等待回调函数处理完成
        if self.frame_event.wait(timeout / 1000.0):
            if self.last_frame is not None:
                return self.last_frame
            else:
                raise Exception("回调函数未返回图像数据")
        else:
            raise Exception("获取图像超时(软件触发)")
...
class MainWindow(QMainWindow):
    ...
    def on_image_detect(self):
        ...
        gray_ng, mean_gray = self.check_mean_gray_ng(frame)
        if gray_ng:
            self.show_window.setPixmap(cv2_to_qpixmap(frame))
            self.picture_box1.setPixmap(cv2_to_qpixmap(frame))
            self.update_result_text(self.text_ai, False)
            self.update_result_text(self.text_trad, False)
            self.update_result_text(self.text_final, False)
            self.update_stats(False)
            self.log_text.append(
                ...
                f"综合: NG | 原因: 平均灰度 {mean_gray:.2f} 不在允许范围内"
            )
            return
        ai_start = time.time()
    ...
    def on_image_side_detect(self):
        ...
        try:
            gray_ng, mean_gray = self.check_mean_gray_ng(roi)
            if gray_ng:
                self.side_window.setPixmap(cv2_to_qpixmap(roi))
                ...
                self.log_text.append(
                    ...
                    f"图片: {os.path.basename(file_path)} | 结果: NG | 原因: 平均灰度 {mean_gray:.2f} 不在允许范围内"
                )
                return
            ai_start = time.time()
