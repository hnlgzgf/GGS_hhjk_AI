...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._plc_prev_trigger_side = False
        self.plc = None
        self.plc_setting = ""
    ...
    def _init_plc_trigger(self):
        """参考 Form1.cs / FpPlc.py：R[0] 上升沿触发一次检测；检测完成后写 R0501(完成脉冲) + R0502(结果)."""
        try:
            ...
            self.log_text.append(f"PLC已连接: {self.plc_setting}（监听 R[0] 上升沿触发）")
    ...
    def _poll_plc_trigger(self):
        ...
        if self._plc_busy:
            # 正在检测，忽略触发电平，直到检测完成
            self._plc_prev_trigger = self.plc.get_r(0)
            return

        trig = self.plc.get_r(0)
        if trig and not self._plc_prev_trigger:
            self.log_text.append("PLC触发：检测一次")
            # 统一走同一套互斥逻辑，避免与手动触发冲突
        ...
        if trig_side and not self._plc_prev_trigger_side:
            self.log_text.append("PLC触发：侧面检测")
            self.side_detect_once(source="PLC")

        self._plc_prev_trigger_side = trig_side

    def _start_detection(self, source: str = "MANUAL"):
        """启动一次检测。检测进行中会忽略新的触发，避免与 PLC/手动冲突。"""
        if self._plc_busy:
        ...
        self.log_text.append(f"{source} 启动检测...")
    ...
    def init_ui(self):
        ...
        self.show_window.setFixedSize(480, 360)
        ...
        self.side_window.setFixedSize(480, 360)
        ...
        self.btn_side_detect.clicked.connect(lambda: self.side_detect_once(source="MANUAL"))
    ...
    def grab_side_frame(self):
        """侧面相机采集图像"""
        try:
            if not self.camera2:
                self.camera2 = HikCamera()
                self.camera2.initialize()

            if not self.camera2.is_opened:
                self.camera2.enum_devices()
                self.camera2.open_device(index=1)

            frame = self.camera2.get_frame()
            return frame
        except Exception as e:
            self.log_text.append(f"侧面相机采集错误: {str(e)}")
            if self.camera2:
                self.camera2.close()
                self.camera2 = None
            raise
    ...
    def crop_side_roi(self, src: np.ndarray) -> np.ndarray:
        rotated = cv2.rotate(src, cv2.ROTATE_180)
        row1, col1, row2, col2 = 680, 800, 1080, 1580
        roi = rotated[row1:row2, col1:col2]
        return roi.copy()
    ...
    def side_detect(self, frame: np.ndarray):
        ...
        results = side_model(frame, conf=0.25, iou=0.7, verbose=False)[0]

        plot_rgb = results.plot()
        plot_bgr = cv2.cvtColor(plot_rgb, cv2.COLOR_RGB2BGR)
    ...
    def side_detect_once(self, source="MANUAL"):
        ...
        try:
            ...
            frame = self.grab_side_frame()

            roi = self.crop_side_roi(frame)

            start = time.time()

            img, ok = self.side_detect(roi.copy())

            cost = time.time() - start

            img = draw_chinese_text(img, "AI侧面检测", (10,10), 16)

            self.side_window.setPixmap(cv2_to_qpixmap(img))

            self.update_result_text(self.text_side, ok)

            if source == "PLC":
                self._plc_report_side(ok)

            self.log_text.append(
                f"侧面检测完成 | {'OK' if ok else 'NG'} | 耗时 {cost:.3f}s"
            )

        except Exception as e:

            self.log_text.append(f"侧面检测错误: {str(e)}")

        finally:

            self._side_busy = False

    def _plc_report_side(self, ok):
        ...
        try:
            ...
            self.plc.write_bit("R0504", (not ok))

            self.plc.write_bit("R0503", True)

            QTimer.singleShot(
                100,
                lambda: self.plc.write_bit("R0503", False) if self.plc else None
            )

        except Exception as e:

            self.log_text.append(f"侧面PLC回写异常: {e}")
